# 👋 Hi, I'm Farouk Osman

![Logo](https://farouk-osman.github.io/Farouk.github.io/farouk-photo.png)

I’m a **Back-End Developer** specialized in **Node.js**, **Express**, and **NestJS**, with a passion for building scalable and efficient web applications.  
I enjoy designing robust APIs, optimizing server performance, and turning ideas into production-ready solutions.

---

## 🚀 About Me

- 💻 Back-End Developer focusing on **Node.js** ecosystem.
- 🛠 Experienced in **Express.js** and **NestJS** frameworks.
- 📦 Built several personal projects covering APIs, authentication, and database integrations.
- 🔍 Always exploring new backend technologies and best practices.
- 📚 Passionate about learning, sharing knowledge, and contributing to open-source.

---

## 🌐 Links

- 📧 **Email:** [faroukosman53@gmail.com](mailto:faroukosman53@gmail.com)
- 💼 **LinkedIn:** [Farouk Osman](https://www.linkedin.com/in/farouk-etman-460032274/)
- 🌍 **Portfolio:** [My Portfolio Website](https://farouk-osman.github.io/Farouk.github.io/#)

---

## 🛠 Tech Stack

- **Languages:** JavaScript, TypeScript
- **Frameworks:** Node.js, Express.js, NestJS
- **Databases:** MongoDB, MySQL
- **Other Tools:** Git, Postman, Docker, RESTful API Design

---

## 📂 Personal Projects

- [**Talabat E-Commerce Website (MEAN Stack)**](https://github.com/Farouk-Osman/Talabat/blob/main/README.md) – Full-stack online shopping platform.
- **Library Booking Website** – Online system for borrowing and managing books.
- **Movie App (IMDb-like)** – Movie listing platform with ratings and reviews.

*(Check my portfolio for more projects!)*

---

## 📫 Contact Me

If you’d like to collaborate or discuss backend development, feel free to reach out via email or connect on LinkedIn.

---

⭐ *Thanks for visiting my profile!*  
